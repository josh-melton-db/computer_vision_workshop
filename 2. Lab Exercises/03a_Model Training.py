...
# cache images data
converter_train = ...(images_train, parquet_row_group_size_bytes=int(bytes_in_train/sc.defaultParallelism))
converter_test = ...(images_test, parquet_row_group_size_bytes=int(bytes_in_test/sc.defaultParallelism))
...
# MAGIC %md
# MAGIC ```
# MAGIC converter_train_vec.make_torch_dataloader(
# MAGIC   transform_spec=torch_spec,
# MAGIC   batch_size=1
# MAGIC )
# MAGIC ```
...
def train_one_epoch(
  ...
  for step in range(steps_per_epoch):
    ...
    pd_batch = ...(train_dataloader_iter) # TODO: get the next value in the iterator
...
def evaluate(
  ...
  for step in range(test_steps):
    ...
    pd_batch = ...(test_dataloader_iter) # TODO: Get the next value in the iterator
    inputs, labels = pd_batch['features'].to(device), pd_batch['label'].to(device)
...
# MAGIC %md
# MAGIC Example of creating a PyTorch Data Loader:
# MAGIC ```
# MAGIC with converter_train.make_torch_dataloader(
# MAGIC       transform_spec=get_transform_spec(is_train=True), 
# MAGIC       batch_size=1
# MAGIC ) as train_dataloader:
# MAGIC ```
...
def train_and_evaluate(lr=0.001, momentum=0.0):
  ...
  with converter_train....(transform_spec=get_transform_spec(is_train=True),  # TODO: create a PyTorch data loader for the training dataset
                                             batch_size=BATCH_SIZE) as train_dataloader, \
       converter_test....(transform_spec=get_transform_spec(is_train=False),   # TODO: create a PyTorch data loader for the test dataset
...
# MAGIC %md
# MAGIC Example of initializing Horovod:
# MAGIC ```
# MAGIC hvd.init()
# MAGIC ```
# MAGIC
# MAGIC Example of distributing an optimizer using Horovod:
# MAGIC ```
# MAGIC optimizer = hvd.DistributedOptimizer(optimizer)
# MAGIC ```
# MAGIC
# MAGIC Example of creating a PyTorch Data Loader:
# MAGIC ```
# MAGIC converter_train.make_torch_dataloader(...)
# MAGIC ```
...
def train_and_evaluate_hvd(lr=0.001, momentum=0.0):
  ...
  hvd....() # TODO: Step 1, initialize Horovod
  ...
  optimizer_hvd = hvd....(optimizer, named_parameters=model.named_parameters()) # TODO: Use Horvod's wrapper to distribute the optimizer
  ...
  with converter_train....( # TODO: create pytorch data loader for training dataset
          transform_spec=get_transform_spec(is_train=True), 
          cur_shard=hvd.rank(), 
          shard_count=hvd.size(),
          batch_size=BATCH_SIZE
          ) as train_dataloader, \
       converter_test....( # TODO: create pytorch data loader for test dataset
...
# MAGIC %md
# MAGIC Example of initializing the Horovod runtime environment, then using it to train and evaluate a model in parallel:
# MAGIC ```
# MAGIC with mlflow.start_run() as run: 
# MAGIC     hr = HorovodRunner(np=hvd_np)
# MAGIC     hr.run(run_training_horovod)
# MAGIC ```
...
hr = ...(np=parallelism) # TODO: Create the runtime environment for Horovod

# run distributed training
with mlflow.start_run(run_name=config['tuned_model_name']) as run:
  
  # argmin contains tuned hyperparameters
  loss, acc, model = ...(..., **argmin) # TODO: train and evaluate the model
